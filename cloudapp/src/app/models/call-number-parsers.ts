...
export interface CallNumberParsers {
  [key: string]: (val: string | Array<string>, item: Item, pattern: string, flags: string, replacement: string) => string | Array<string>
}
...
export const callNumberParsers: CallNumberParsers = {
  ...
  'reg_exp': (val, item, pattern, flags, replacement) => {
    let wasArray = false;
    if (Array.isArray(val)) {
      val = val.join(' ');
      wasArray = true;
